                ...
                Tables\Actions\Action::make('sold_out')
                    ...
                    ->form([
                        ...
                        Forms\Components\TextInput::make('sold_commission')
                            ->numeric()
                            ->required()
                            ->default(fn (Model $record) => $record->sale_price_type == PropertyPriceType::Fix ? ($record->sale_price_from * $record->sale_owner_commission / 100) : 0)
                            ->rules([
                                'required',
                                'numeric',
                                'integer',
                    ...
                    ->action(function (Model $record, Tables\Actions\Action $action): void {
                        $record->update([
                            'status' => $record->rentable ? PropertyStatus::SoldOut : PropertyStatus::Completed,
                        ]);
                        $action->success();
                    }),
                Tables\Actions\Action::make('rented')
                    ...
                    ->form([
                        ...
                        Forms\Components\TextInput::make('rented_commission')
                            ->numeric()
                            ->required()
                            ->default(fn (Model $record) => $record->rent_price_type == PropertyPriceType::Fix ? ($record->rent_price_from * $record->rent_owner_commission / 100) + ($record->rent_price_from * $record->rent_customer_commission / 100) : 0)
                            ->rules([
                                'required',
                                'numeric',
                                'integer',
                    ...
                    ->action(function (Model $record, Tables\Actions\Action $action): void {
                        $record->update([
                            'status' => $record->saleable ? PropertyStatus::Rented : PropertyStatus::Completed,
                        ]);
